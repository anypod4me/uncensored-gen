...
export default function FluxGenerator() {
  ...
  const generateImage = () => {
    ...
    // 2. Build the Fixed API URL
    // Added "/p/" and encodeURIComponent so spaces don't break the link
    ...
    // 3. Pre-loading the image to ensure the spinner stays until it's ready
    const img = new Image();
    img.src = fullUrl;
    img.onload = () => {
      setImageUrl(fullUrl);
